...
export function getAverageRGB(imgEl: HTMLImageElement): { r: number, g: number, b: number } {
  ...
  const height: number = canvas.height = imgEl.naturalHeight || imgEl.offsetHeight || imgEl.height;
  const width: number = canvas.width = imgEl.naturalWidth || imgEl.offsetWidth || imgEl.width;
...
export async function getAccentHEXColor(image: string): Promise<string> {
  ...
  const hexColor = rgbaToInt(accentColor[0], accentColor[1], accentColor[2], 255).toString(16).slice(1).toUpperCase();

  return `#${hexColor}`;
}

export async function getAverageRGBColor(base64: string): Promise<{ r: number; g: number; b: number }> {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = "Anonymous"; // Avoid CORS issues
    img.src = base64;
