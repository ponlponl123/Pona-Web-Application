...
export async function GET(
  ...
  else tempUnit = searchParams.get('tmp_unit') || 'celsius';

  const callOpenMeteo = await axios.get(
    `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min,sunset&temperature_unit=${tempUnit}&timeformat=unixtime&timezone=Asia%2FBangkok&past_days=1&forecast_days=1`
  );
  ...
  try {
    ...
    const { city, state, country } =
      geocodeResponse.results[0].address_components.reduce(
        (
