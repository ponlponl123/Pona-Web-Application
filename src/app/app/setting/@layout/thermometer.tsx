...
import {
  Thermometer as ThermometerType,
...
function Thermometer() {
  ...
  return (
    <section
      ...
      id='layout-thermometer'
      data-section
    >
      <h2 className='text-3xl flex items-center gap-4 pt-4'>
        <ThermometerIcon weight='fill' />
        {language.data.app.setting.layout.thermometer.title}
      </h2>
      <RadioGroup
        className='w-full'
        defaultValue={userSetting.thermometer}
        onValueChange={value => {
          setUserSetting({
            ...userSetting,
            thermometer: value as ThermometerType,
          });
        }}
      >
        <Radio value='c'>
