  ...
  return (
    <div className='flex flex-col gap-4' id='layout-theme' data-section>
            ...
            <div className='gap-2 flex flex-wrap pt-6'>
              {
                fetchThemes(true, currentTheme.isAmoled).map((theme, index) => (
                  <ThemeDot key={index} theme={theme} active={(currentTheme.single === theme)} onClick={() => {setCurrentTheme((value)=>{const n_theme={...value,single:theme};setTheme(n_theme);return n_theme})}} />
                ))
              }
            </div>
          </section>
          ...
          <div className='w-full flex max-xl:flex-wrap gap-4'>
            <section className='p-6 border-3 rounded-2xl border-warning w-full'>
              ...
              <div className='gap-2 flex flex-wrap pt-6'>
                {
                  fetchThemes(true, currentTheme.isAmoled).map((theme, index) => (
                    <ThemeDot key={index} theme={theme} active={(currentTheme.day === theme)} onClick={() => {setCurrentTheme((value)=>{const n_theme={...value,day:theme};setTheme(n_theme);return n_theme})}} />
                  ))
                }
              </div>
            </section>
            <section className='p-6 border-3 rounded-2xl border-secondary w-full'>
              ...
              <div className='gap-2 flex flex-wrap pt-6'>
                {
                  fetchThemes(true, currentTheme.isAmoled).map((theme, index) => (
                    <ThemeDot key={index} theme={theme} active={(currentTheme.night === theme)} onClick={() => {setCurrentTheme((value)=>{const n_theme={...value,night:theme};setTheme(n_theme);return n_theme})}} />
                  ))
                }
              </div>
            </section>
      ...
      <Switch
        ...
        onValueChange={(value) => {
          setCurrentTheme((ref_value)=>{const n_theme={...ref_value,sync:value};setTheme(n_theme);return n_theme})
        }}
      />
      {
        (
