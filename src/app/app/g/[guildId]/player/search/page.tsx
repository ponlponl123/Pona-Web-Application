...
import { Button, Chip, Image, Link, Progress, ScrollShadow, Tab, Tabs, Tooltip } from '@nextui-org/react';
import { CaretRight, Cloud, FlyingSaucer, Heart, MagnifyingGlass, ShareFat } from '@phosphor-icons/react/dist/ssr';
...
function Page() {
  ...
  React.useEffect(() => {
    const letSearch = async () => {
      ...
      const orderedResult = orderedKeys.reduce((acc: { [key in typeof orderedKeys[number]]: HTTP_SearchResult[] }, key) => {
        if (sortedResult[key]) {
      ...
      try {
        if ( orderedResult["Top result"] ) {
          const topResult = orderedResult["Top result"][0] as unknown as TopResults;
          if ( topResult.resultType === 'artist' || topResult.resultType === 'artist-detail' ) {
            const accentColor = await getAccentHEXColorFromUrl('/api/proxy/image?r='+topResult.thumbnails[0].url);
  ...
  return (
    <div className='w-full max-w-screen-md mx-auto mt-24 gap-4 flex flex-col items-center justify-center text-center pb-[16vh]'>
      ...
      <div id='pona-search-result' className='w-full flex flex-col gap-12 mt-4'>
        {
          !loading &&
            ((!filter || filter === 'all') && searchResult) ?
                            ...
                            <div className='flex gap-6 p-6 max-md:p-4 bg-foreground/5 rounded-3xl relative overflow-hidden'>
                              ...
                              <div className='w-28 h-28 md:min-w-28 md:min-h-28 max-md:h-16 max-md:w-16 max-md:min-w-16 max-md:min-h-16 relative overflow-hidden flex-[0 1 auto]'>
                                <Image src={`/api/proxy/image?r=${track.thumbnails[track.thumbnails.length-1].url}`} alt={trackTitle} classNames={{
                                  wrapper: 'max-w-none',
                                  img: track.resultType==='artist'||track.resultType==='artist-detail' ? 'rounded-full' : undefined
                              ...
                              <div className='flex flex-col gap-1 items-start justify-center z-10 w-0 min-w-0 flex-1'>
                                {
                                  track.resultType === 'artist' && track.artists[0].id === 'UCsuWLo_X3E2c31LUI0nZK-w' &&
                                  <Chip size='sm' className='tracking-wider bg-[#a7b9df]' startContent={<Cloud size={12} className='mx-1' weight='fill' />}>{language.data.spacial.little_angel}</Chip>
                                }
                                {
                                  track.resultType === 'song' ||
                                  track.resultType === 'video' ?
                                ...
                                <div className='flex flex-row gap-3 items-center justify-start w-full mt-1'>
                                  {
                                    ...
                                    <>
                                      ...
                                      <Button href={`/app/g/${guild?.id}/player/c?c=${track?.artists[0].id}`} size='sm' onPress={()=>{router.push(`/app/g/${guild?.id}/player/c?c=${track?.artists[0].id}`)}} color='default' className='bg-opacity-40' radius='full' isIconOnly><CaretRight weight='bold' /></Button>
                                    </>
                                  }
                                </div>
                              </div>
            ...
            : (!loading && filter && searchResult && searchResult[filterNormallize] && searchResult[filterNormallize].length > 0) ?
