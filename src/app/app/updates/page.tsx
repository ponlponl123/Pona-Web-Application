    ...
    <main id='app-panel'>
      <main id='app-workspace'>
        ...
        <div className='flex mt-12 justify-between gap-12 lg:flex-wrap items-center'>
          <h2 className='text-5xl flex gap-4 items-center'>
          ...
          <Button
            ...
            radius='full'
            size='lg'
            className='max-lg:p-4 max-lg:min-w-max'
          >
        ...
        <SubscribeModal isOpen={isOpen} onOpenChange={onOpenChange}>
          <></>
        </SubscribeModal>
        {isLoading ? (
            ...
            {data.map((note: notes, index: number) => (
              ...
              >
                <Chip
                  ...
                  className='text-base'
                  color={
                    note.tag.toLowerCase() === 'pre-release'
                      ? 'warning'
                      : note.tag.toLowerCase() === 'release'
                        ? 'primary'
                        : 'default'
                  }
                >
...
function ModalContext({
  ...
  setSelectedNote: React.Dispatch<
    React.SetStateAction<{ note: notes; version: string } | null>
  >;
}) {
  const { language } = useLanguageContext();
    ...
    (async () => {
      try {
        const response = await fetch(
          `/api/patchnote/${selectedNote.note.tag}/${selectedNote.version}`
        );
        if (!response.ok) {
          throw new Error('HTTP error!');
        }
        ...
        {onNoteClose => (
          <>
            <ModalHeader className='flex items-center gap-3'>
              <Chip
                color={
                  selectedNote.note.tag.toLowerCase() === 'pre-release'
                    ? 'warning'
                    : selectedNote.note.tag.toLowerCase() === 'release'
                      ? 'primary'
                      : 'default'
                }
              >
                {(language.data.app.updates.translate as Translations)[
                  selectedNote.note.tag.toLowerCase()
                ]
                  ? (language.data.app.updates.translate as Translations)[
                      selectedNote.note.tag.toLowerCase()
                    ]
              ...
              {language.data.app.updates.version}{' '}
              {selectedNote.version.replace('.md', '')}
              {authorMatch && (
                <Link
                  className='flex items-center ml-auto rounded-full p-1 relative'
                  href={`https://github.com/${authorMatch[1]}`}
                  target='_blank'
                >
                  <Avatar
                    className='h-8 w-8'
                    size='sm'
                    src={`https://github.com/${authorMatch[1]}.png`}
                    alt={`Author: ${authorMatch[1]}`}
                  />
                  <div className='flex flex-col gap-0 mx-2'>
                    <h1 className='m-0 text-sm leading-none'>
                      {authorMatch[1]}
                    </h1>
                    <span className='text-[10px] leading-none font-bold text-foreground/40'>
                      Author
                    </span>
                  </div>
                </Link>
            ...
            <ModalFooter className='items-center'>
              <Snippet
                className='rounded-2xl w-max max-w-[66%] overflow-hidden'
                classNames={{
                ...
                color='default'
                size='sm'
                symbol={<IconLink weight='bold' size={14} />}
              >{`/app/updates/${selectedNote.note.tag}/${selectedNote.version.replace('.md', '')}`}</Snippet>
              <Button
                ...
                onPress={onNoteClose}
              >
                {language.data.app.updates.subscription.modal.close}
              </Button>
            </ModalFooter>
          </>
        )}
