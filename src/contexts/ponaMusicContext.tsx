...
const decodeBase64JSON = <T,>(base64Str: string): T => {
      ...
      if (!isConnected) {
        ...
        const connectSocket = () => {
          if (retryCount >= maxRetries) {
            console.error(
              "Max retry attempts reached. Could not connect to socket."
            )
            return
          }
          retryCount++
          ...
          iosocket.on("handshake", async (ponaState: string) => {
            // ใช้ Helper Function แทน Buffer
            const decodedPonaState = decodeBase64JSON<{
          ...
          iosocket.on("pause_updated", (paused: number) => {
            setPonaCommonState((value) => {
              if (value) {
                return {
                  ...value,
                  pona: { ...value.pona, paused: paused === 1 },
                }
              }
              return value
            })
          ...
          iosocket.on("repeat_updated", (repeatState: string) => {
            const decodedRepeatState =
              decodeBase64JSON<HTTP_PonaRepeatState>(repeatState)
            setPonaCommonState((value) => {
              if (value) {
                return {
                  ...value,
          ...
          iosocket.on("track_started", async (track: string) => {
            let decodedTrack = decodeBase64JSON<Track>(track)
          ...
          iosocket.on("queue_updated", async (queue: string) => {
            const decodedQueue = decodeBase64JSON<Queue>(queue)
          ...
          iosocket.on("player_created", (pona: string) => {
            const decodedPona =
              decodeBase64JSON<HTTP_PonaCommonStateWithTracks | null>(pona)
          ...
          iosocket.on("member_state_updated", (memberVoiceState: string) => {
            const decodedMemberVoiceState =
              decodeBase64JSON<MemberVoiceChangedState>(memberVoiceState)
            if (
