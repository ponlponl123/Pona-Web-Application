...
export const PonaMusicCacheContextProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [subscribe_state_cache, SetSubscribeStateCache] = useState<
    subscribeState[]
  >([]);
  const [favorite_state_cache, SetFavoriteStateCache] = useState<
    favoriteState[]
  >([]);
  const [relatedInfoCache, SetRelatedInfoCache] = useState<relatedInfo>({
    videoId: '',
    watch_playlist: undefined,
    related: undefined,
  });
  ...
  const GetSubscribeStateFromChannelId = async (channelId: string) => {
    const state = subscribe_state_cache.find(
