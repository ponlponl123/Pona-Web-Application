...
function TrackList({
  ...
  return (
    <div
      className={
        'w-full max-w-full flex gap-4 items-center justify-start group py-2 px-4 rounded-2xl overflow-hidden group' +
    ...
    >
      <div className='flex flex-row gap-1 justify-center items-center w-12 h-12 min-w-12 max-w-12 max-h-12 relative flex-[0 1 auto]'>
        <PlayButton
          playPause={ponaCommonState?.current?.identifier === data.videoId}
          className={
            'rounded-xl absolute top-0 left-0 bg-transparent ' +
            ` ${ponaCommonState?.current?.identifier === data.videoId ? '' : 'group-hover:opacity-100 opacity-0'}`
          }
          iconSize={12}
          classNames={{
            playpause: 'text-sm',
      ...
      <div className='flex flex-col gap-1 justify-center items-start flex-1 w-0 min-w-0 opacity-40 group-hover:opacity-100'>
        <h1 className='text-xl max-w-full w-full overflow-hidden overflow-ellipsis whitespace-nowrap text-start'>
          {data?.title}
        </h1>
