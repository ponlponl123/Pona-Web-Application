...
function Button({
  ...
  let handleEffect = (event: React.MouseEvent<HTMLButtonElement>): unknown => {
    return event
  }
  ...
  return (
    <button
      className={`handwritting-btn handwritting-btn-${style} handwritting-btn-${variant} handwritting-btn-size-${size} group ${className}`}
      type="button"
      onClick={Clicked}
    >
      {style === "rounded" ? (
