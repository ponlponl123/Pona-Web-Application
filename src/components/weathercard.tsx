...
const WeatherCard: React.FC = () => {
  ...
  const [weatherData, setWeatherData] = React.useState<ResponseData | null>(
    null
  );
  ...
  const WeatherIcon: React.FC<IconProps> =
    weatherData && weatherData.data.daily.weather_code[1]
      ? WMO_CODES_ICONS['wmo-code'][weatherData.data.daily.weather_code[1]]
      : Question;
  const diffTemp =
    (weatherData?.data.current.temperature_2m ?? 0) -
    (weatherData?.data.daily.temperature_2m_max[1] ?? 0);
          ...
          <div className='flex gap-4 items-center justify-between'>
            ...
            <div className='flex flex-col items-end justify-center gap-1'>
              <h1 className='text-sm'>
                {
                  language.data.app.home.weather.status['wmo-code'][
                    weatherData?.data.daily
                      .weather_code[1] as unknown as keyof (typeof language.data.app.home.weather.status)['wmo-code']
                  ]
                }
              </h1>
              <h3 className='flex text-sm gap-1 items-center'>
                <span className='flex items-center'>
                  <ArrowUp /> {weatherData?.data.daily.temperature_2m_max[1]}
                </span>
                /
                <span className='flex items-center'>
                  <ArrowDown /> {weatherData?.data.daily.temperature_2m_min[1]}
                </span>
              </h3>
            </div>
          </div>
          ...
          <div className='flex flex-col gap-4'>
            <div className='flex flex-col'>
              <h2 className='text-sm text-foreground/60'>
                {language.data.app.home.weather.today_temp}
              </h2>
              <h1 className='font-bold text-base'>
                {diffTemp >= 0 && diffTemp <= 1.5
                  ? language.data.app.home.weather.temp_status
                      .little_higher_than_yesterday
                  : diffTemp >= -1.5 && diffTemp <= 0
                    ? language.data.app.home.weather.temp_status
                        .little_lower_than_yesterday
                    : diffTemp <= -1.5 && diffTemp <= 0
                      ? language.data.app.home.weather.temp_status
                          .lower_than_yesterday
                      : diffTemp >= 1.5 && diffTemp >= 0
                        ? language.data.app.home.weather.temp_status
                            .higher_than_yesterday
                        : language.data.app.home.weather.temp_status
                            .like_yesterday}
              </h1>
            </div>
            <div className='flex flex-col'>
              <h2 className='text-sm text-foreground/60'>
                {language.data.app.home.weather.dontmisssunset}
              </h2>
              <h1 className='font-bold text-base'>
                {language.data.app.home.weather.sunsetwillbeat}{' '}
                {userSetting.timeformat === 12
                  ? weatherData?.data.daily.sunset[1]
                    ? new Date(
                        weatherData.data.daily.sunset[1]
