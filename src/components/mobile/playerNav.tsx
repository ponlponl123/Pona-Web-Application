...
function PlayerNav() {
  ...
  return (
    <div className='w-full h-16 fixed bottom-0 left-0 bg-background/90 backdrop-blur-md border-t-[2px] border-foreground/10 flex flex-row items-center justify-around z-50 md:hidden'>
      <Link
        href={homeHref}
        className={`w-full h-full flex flex-col items-center justify-center gap-1 ${
          pathname === homeHref ? 'text-primary' : 'text-muted-foreground'
        }`}
      >
        <HouseSimple size={20} />
      ...
      <Link
        href={browseHref}
        className={`w-full h-full flex flex-col items-center justify-center gap-1 ${
          pathname === browseHref ? 'text-primary' : 'text-muted-foreground'
        }`}
      >
        <Compass size={20} />
      ...
      <Link
        href={historyHref}
        className={`w-full h-full flex flex-col items-center justify-center gap-1 ${
          pathname === historyHref ? 'text-primary' : 'text-muted-foreground'
        }`}
      >
